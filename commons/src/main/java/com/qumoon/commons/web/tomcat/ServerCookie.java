package com.qumoon.commons.web.tomcat;
...
/**
 * 这个类从Tomcat 6.0.29的<code>org.apache.tomcat.util.http.ServerCookie</code>改编。 <ul> <li>删除了不需要的代码。</li>
 * <li>当value为空时，不显示为""。</li> <li>增加了Cookie Version=1时，Discard支持。</li> </ul> <p> Server-side cookie representation.
 * Allows recycling and uses MessageBytes as low-level representation ( and thus the byte-> char conversion can be
 * delayed until we know the charset ). Tomcat.core uses this recyclable object to represent cookies, and the facade
 * will convert it to the external representation. </p>
 */
