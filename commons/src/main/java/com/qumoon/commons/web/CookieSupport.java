...
public class CookieSupport extends Cookie {
  ...
  private static FastMethod getHttpOnlyMethod() {
    ...
    if (m != null) {
      log.debug(
          "Method Cookie.isHttpOnly() defined in current version of servlet api.  CookieSupport will make use of it.");
  ...
  /**
   * 生成set-cookie header的值，即使cookie value中包含非法值，也不会报错。 <p> 请不要使用<code>toString()</code>方法来生成cookie header，而应该使用
   * <code>getCookieHeaderValue()</code>来取代。 </p>
   */
  @Override
